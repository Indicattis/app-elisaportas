...
BEGIN
  ...
  DELETE FROM correcao_linhas
   WHERE correcao_id IN (SELECT id FROM correcoes WHERE pedido_id = p_pedido_id);
  DELETE FROM correcoes WHERE pedido_id = p_pedido_id;

  UPDATE pedidos_producao SET pedido_origem_id = NULL WHERE pedido_origem_id = p_pedido_id;
  ...
  DELETE FROM pedido_comentarios WHERE pedido_id = p_pedido_id;
